...
public class StpCdcRecordStoreUnawareBucketMultiWriteOperator
                ...
                Executors.newSingleThreadScheduledExecutor(
                        new ExecutorThreadFactory(
                                Thread.currentThread().getName()
                                        + "-StpCdcRecordStoreUnawareBucketMultiWrite-Compaction"));
    }
    ...
    public void processElement(StreamRecord<CdcMultiplexRecord> streamRecord) throws Exception {
        ...
        if (!optionalConverted.isPresent()) {
            ...
            for (int retry = 0; retry < retryCnt; ++retry) {
                ...
                optionalConverted = toGenericRow(record.record(), latestTable, logCorruptRecord);
                if (optionalConverted.isPresent()) {
                    break;
                }
                Thread.sleep(
    ...
    public static class Factory
        ...
        public <T extends StreamOperator<MultiTableCommittable>> T createStreamOperator(
                StreamOperatorParameters<MultiTableCommittable> parameters) {
            return (T)
                    new StpCdcRecordStoreUnawareBucketMultiWriteOperator(
                            parameters,
                            catalogLoader,
